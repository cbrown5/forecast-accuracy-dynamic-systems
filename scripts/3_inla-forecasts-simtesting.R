# Simulation testing of legacy vs modern split methods
# 2024-11-30
...
nsims <- 20 #number of simulation replicates for each scneario
...
do_sim <- function(isim, ispp, imodel){
  ...
  xall_fixed <- fit_models_1site(ispp, imodel, dat2, t1_train, tn_train, t0_test_vals, 
                                 ...
                                 split_name = "fixed", save_plots = FALSE, 
  ...
  for (iorigin in t0_test_vals){
    ...
    xout <- fit_models_1site(ispp, imodel, dat2, t1_train_rolling, iorigin, iorigin, 
                             save_model_fits = FALSE,
                             split_name = "rolling", save_plots = FALSE, 
                             central_stat_to_use = central_stat_to_use)
...
#drop NA rows
datout2 <- datout[-which(is.na(datout$isim)),]

save(datout2, file = "Outputs/AR1-sim-results-2025-01-010.rda")

datout2 %>%
  filter(imodel == "ar1") %>%
  mutate(split_period = paste(split_name, t0_test)) %>% 
  ggplot() + 
  aes(x = horizon, y = mase2, color = t0_test, 
      fill = t0_test,group = split_period,
      linetype = split_name) + 
  stat_smooth(method = "gam", formula = y ~ s(x, k = 4), se = TRUE) + 
  scale_linetype_manual("Split approach", values = c("solid", "dotted")) +
  stat_smooth() + 
  facet_grid(t0_test~ispp)
