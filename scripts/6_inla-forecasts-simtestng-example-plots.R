...
nsims <-6 #number of simulation replicates for each scneario
...
g1 <- datout %>%
  mutate(Split_rule = 
    if_else(split_name == "fixed", "Legacy", "Modern")) %>%
  filter(!is.na(y2)) %>%
  ggplot() + 
    aes(x = year, y = y2) + 
    geom_point(alpha = 0.5) + 
    geom_line(aes(y = med, color = t0_test,
      group = t0_test), size = 1.5, alpha = 0.5) +
    scale_color_continuous("Forecast origin",
      low = "#eb4ae0", high = "#330053") +
    facet_grid(isim~Split_rule) +
...
ggsave(g1, filename = 
  "Outputs/figures-methods-ms/2025-03-21-example-simulation-fits.png", width = 6, height = 8)
